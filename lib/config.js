 ...
 * Used for funnel.json and other build artifacts.
 */
...
/**
 * Check if a CLI flag is present in an argv array.
 * Only checks args after the first two entries (node binary + script path).
 *
 * @param {string[]} argv - process.argv or equivalent
 * @param {string}   flag - Flag to check (e.g. '--lenient', '--strict')
 * @returns {boolean}
 */
...
/**
 * Format a build summary line (shared by build + dev actions).
 *
 * @param {number} built       - Pages built
 * @param {number} totalErrors - Combined render + funnel errors
 * @param {string} timing      - Human-readable timing string
 * @returns {string}
 */
function formatBuildSummary(built, totalErrors, timing) {
    const pageLabel = `${built} page${built !== 1 ? 's' : ''}`;
    const errorSuffix = totalErrors
        ? ` (${totalErrors} error${totalErrors !== 1 ? 's' : ''})`
        : '';
    return `Built ${pageLabel} in ${timing}${errorSuffix}`;
}
