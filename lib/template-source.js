...
function assertValidSource(repo, ref, source_path) {
    ...
    if (source_path.includes('..') || source_path.includes('\0') || source_path.startsWith('~')
        || normalized.startsWith('..') || path.posix.isAbsolute(normalized)) {
        throw invalidSource(`invalid source path "${source_path}" — must be a relative subtree with no ".." segments`);
    }
}
