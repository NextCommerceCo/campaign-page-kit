    ...
    return new Promise((resolve, reject) => {
        get(url, (res) => {
            if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
                ...
                return resolve(fetchBuffer(res.headers.location, redirects - 1, get));
            }
...
function resolveLocalRoot(source, ctx) {
    ...
    if (p === '~' || p.startsWith('~/')) {
        ...
        // `~user` (another user's home) is not expanded — reject clearly rather
        // than silently treating it as a relative path that won't be found.
        throw codedError('INVALID_INPUT', `"~user" home expansion is not supported: ${p}`);
    }
...
async function cloneToTemp(source, ctx) {
    ...
    try {
        ...
        if (err && err.killed) {
            throw codedError('UPSTREAM_FETCH_FAILED', `git clone timed out after ${GIT_CLONE_TIMEOUT_MS / 1000}s`);
        }
...
// `timeout` guards against a hung SSH connection blocking campaign-init forever;
// on expiry the child is killed (SIGTERM) and the callback err has killed=true.
...
// github: read a GitHub repo over HTTPS — no auth, no clone. Reads are cheap
// per-file raw fetches; the tarball is downloaded only on materialize() so the
// picker never pulls a tarball just to list templates. (The built-in `public`
// source uses this transport, but "public" is the source's name, not the type.)
class GithubProvider {
    ...
    _tarballUrl() {
        // Bare ref resolves branches, tags, and SHAs alike (refs/heads/<ref>
        // would 404 for a tag); matches how _rawUrl uses the ref directly.
        return `https://codeload.github.com/${this.source.repo}/tar.gz/${this.source.ref}`;
    }
    async readJson(rel) {
...
const SOURCE_TYPES = {
    github: (source, ctx) => new GithubProvider(source, ctx),
