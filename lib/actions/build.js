...
Flags:
  ...
  --no-clean    keep existing files in _site/ before building
  --verbose     print debug diagnostics to stderr
  --help, -h    show this help and exit 0

By default, campaign-build cleans _site/ before a full build so removed source
pages and assets do not remain in publish output. Use --no-clean only when you
intentionally manage additional files inside _site/.
...
async function main() {
    ...
    const summary = await build({ clean, verbose });
