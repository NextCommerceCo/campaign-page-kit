...
function parseStrict(argv) {
    return argv.slice(2).includes('--strict');
}
...
async function runDevServer(campaigns) {
    ...
    const strict = parseStrict(process.argv);
    const lenient = !strict; // Dev mode is lenient by default; use --strict to validate

    console.log('');

    // Initial build
    try {
        const { built, errors, funnelErrors = 0, ms } = await build({ campaigns, mode: 'development', lenient });
    ...
    const { server, watcher } = serve({
        ...
        onRebuild: async (changedPath) => {
            ...
            const { built, errors, funnelErrors = 0, ms } = await build({ campaigns: { [slug]: campaigns[slug] }, files, mode: 'development', lenient });
            const timing = ms < 1000 ? `${ms}ms` : `${(ms / 1000).toFixed(2)}s`;
...
module.exports = { runDevServer, parsePort, parseStrict };
