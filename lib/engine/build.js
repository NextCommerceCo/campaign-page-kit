...
async function build(opts = {}) {
    ...
    if (isPartialRebuild) {
        logger.debug('Partial rebuild — skipping funnel regeneration (only full builds update funnel.json)');
    }
    let funnelErrors = 0;
    for (const [slug, pages] of isPartialRebuild ? [] : campaignPages) {
        ...
        if (lenient) {
            // Downgrade errors to warnings (log in same order as JSON: errors first, then warnings)
        ...
        // Write funnel.json (never fatal, always warning-only)
        try {
            const jsonPath = writeFunnelJson(json, cpkPath, slug);
            const relJson = path.relative(process.cwd(), jsonPath);
            logger.info(`Funnel map: \x1b[36m${relJson}\x1b[0m`);
        } catch (e) {
            logger.warn(`Funnel JSON write failed for [${slug}]: ${e.message}`);
        }
    }
