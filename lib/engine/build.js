 ...
 * @param {boolean}  [opts.clean]      - Remove outputPath before a full build.
...
function cleanOutputPath(outputPath, opts = {}) {
    ...
    for (const protectedPath of opts.protectedPaths || []) {
        const protectedResolved = path.resolve(protectedPath);
        if (pathsOverlap(resolved, protectedResolved)) {
            throw new Error(`Refusing to clean output path that overlaps source path: ${resolved}`);
        }
    }
...
function pathsOverlap(a, b) {
    ...
    return !aToB || !bToA ||
        (!aToB.startsWith('..') && !path.isAbsolute(aToB)) ||
...
module.exports = { build, cleanOutputPath, resolveOutput };
